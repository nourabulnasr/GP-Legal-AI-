...
def require_verified_lawyer(current_user: User = Depends(get_current_user)) -> User:
    """Dependency that allows only verified lawyers or admins."""
    user_type = getattr(current_user, "user_type", "user")
    role = getattr(current_user, "role", "user")
    if user_type != "lawyer" and role != "admin":
