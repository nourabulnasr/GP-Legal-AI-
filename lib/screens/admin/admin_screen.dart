...
class _AdminScreenState extends State<AdminScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadLawyerApps() async {
    ...
    try {
      ...
      // Merge locally-queued applications (submitted when /lawyer/apply was 404).
      // Local entries take precedence over backend entries for the same user_id.
      final localAll = await LocalLawyerQueue.getAll();
      final filter = _appStatusFilter;
      final localFiltered = localAll.where((m) {
        final s = m['status']?.toString() ?? 'pending';
        if (filter == 'all') return true;
        if (filter == 'pending') return s == 'pending';
        return s == filter;
      }).toList();
      final localUserIds = localFiltered.map((m) => m['user_id'] as int?).toSet();
      final backendOnly = backendApps.where((a) {
        return !localUserIds.contains((a as Map<String, dynamic>)['user_id'] as int?);
      }).toList();
      if (mounted) setState(() => _lawyerApps = [...localFiltered, ...backendOnly]);
    } catch (e) {
      if (mounted) {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context)
            .showSnackBar(SnackBar(content: Text('Role → $role')));
      }
    } on ApiException catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(e.message)));
  ...
  Future<Uint8List?> _fetchLawyerFileBytes(int appId, String fileType) async {
    ...
    // Local queue entries store the bytes directly (appId == userId for queue entries).
    try {
      final localBytes = await LocalLawyerQueue.getFileBytes(appId, fileType);
      if (localBytes != null && localBytes.isNotEmpty) return localBytes;
    } catch (_) {}

    // Fall back to the backend download endpoint.
    try {
      return fileType == 'cv'
  ...
  Future<void> _reviewLawyerApp(int appId, int? userId, String userEmail, String action) async {
    ...
    // Backend failures are non-fatal — the local queue is the source of truth for
    // locally submitted applications, and is always updated below.
    try {
      ...
      if (reviewErr.statusCode == 404 && userId != null) {
        try {
          ...
          // local queue update (below) is the fallback.
        }
      }
      // Non-404 review endpoint errors are swallowed; we still update locally.
    }

    // On approval also set user_type = 'lawyer' on the backend.
    if (action == 'approve' && userId != null) {
      try { await legato.adminUpdateUserType(userId, 'lawyer'); } catch (_) {}
    }

    // Always update local queue — definitive source for locally submitted apps.
    if (userId != null) {
      try {
        await LocalLawyerQueue.setStatus(userId, newStatus, adminNote: note);
      } catch (_) {}
    }
                          ...
                          child: _lawyerApps.isEmpty
                              ...
                              : ListView.separated(
                                  ...
                                  separatorBuilder: (_, i) =>
                                      const Divider(height: 1),
                                  itemBuilder: (context, i) {
                                    final app = _lawyerApps[i]
                                        as Map<String, dynamic>;
                                    final appId = app['id'] as int;
                                    final email =
                                        app['user_email']?.toString() ?? '';
                                    final licNo =
                                        app['bar_license_number']?.toString() ??
                                            '';
                                    final docFile =
                                        app['document_filename']?.toString() ??
                                            '';
                                    final hasDoc =
                                        app['has_document'] as bool? ?? false;
                                    final cvFile =
                                        app['cv_filename']?.toString() ?? '';
                                    final hasCv =
                                        app['has_cv'] as bool? ?? false;
                                    final idCardFile =
                                        app['id_card_filename']?.toString() ??
                                            '';
                                    final hasIdCard =
                                        app['has_id_card'] as bool? ?? false;
                                    final status =
                                        app['status']?.toString() ?? 'pending';
                                    final adminNote =
                                        app['admin_note']?.toString() ?? '';
                                              ...
                                              Row(
                                                children: [
                                                  ...
                                                  TextButton(
                                                    ...
                                                    onPressed: () => _viewLawyerFile(
                                                        appId, 'id-card',
                                                        idCardFile.isNotEmpty ? idCardFile : 'id_card'),
                                                    child: const Text('View'),
                                                  ),
                                                  TextButton(
                                                    style: TextButton.styleFrom(
