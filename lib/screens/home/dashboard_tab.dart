...
class DashboardTabState extends State<DashboardTab> {
  ...
  Future<void> _loadPendingAppsIfAdmin() async {
    ...
    try {
      ...
      if (mounted) setState(() => _pendingLawyerAppsCount = apps.length);
      ...
      if (e.statusCode == 404) {
        ...
        try {
          ...
          if (mounted) setState(() => _pendingLawyerAppsCount = count);
        } catch (_) {}
      }
    } catch (_) {}
  }
...
class _LawyerStatusBannerState extends State<_LawyerStatusBanner> {
  ...
  Future<void> _fetchStatus() async {
    ...
    try {
      ...
      if (e.statusCode == 404) {
        // /lawyer/status not deployed — fall back to the user model.
        if (!mounted) return;
        final lawyerStatus = authProv.user?.lawyerStatus ?? 'pending';
        setState(() => _data = {'status': lawyerStatus});
